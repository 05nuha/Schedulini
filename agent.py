...
def extract_json_from_history(raw_list):
    ...
    for item in raw_list:
        try:
            # Attempt to parse the entire item as JSON
            json_data = json.loads(item)
            if isinstance(json_data, list):
                json_data_list.append(item)
        except json.JSONDecodeError:
            # If parsing fails, attempt to extract JSON manually
    ...
    try:
        for raw_json in raw_json_list:
            try:
                ...
                print(f"❌ Failed to parse JSON: {raw_json}. Error: {e}")
                continue
            ...
            for course in json_data:
                try:
                    ...
                    print(f"❌ Error creating course object: {course}. Error: {e}")
        return linked_list
    except Exception as e:
    ...
    with open(filename, "w", newline="", encoding="utf-8") as file:
        ...
        while current:
            ...
            writer.writerow([
                ...
                str(course.end_time),
                str(course.max_enrollment),
                str(course.total_enrollment)
            ])
            current = current.next
...
async def main():
    ...
    task = f'''1. Go to CUD portal login page
        2. Log in with username: {username} and password: {password}
        3. Select term: {selected_term} from the dropdown menu
        4. Navigate to "Course Offerings"
        5. Click on "Show Filter"
        6. Select "SEAST" from divisions
        7. Click on "Apply filter" and WAIT for full load.
        8. After the filter is applied and courses are visible:
